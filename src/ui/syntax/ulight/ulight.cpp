...
std::u8string_view html_entity_of(char8_t c) {
  ...
  default:
    ULIGHT_DEBUG_ASSERT_UNREACHABLE(
        u8"We only support a handful of characters.");
  }
}
...
void append_html_escaped(Non_Owning_Buffer<char> &out, std::string_view text) {
  ...
  while (!text.empty()) {
    const std::size_t bracket_pos =
        text.find_first_of(as_string_view(escaped_chars));
...
[[nodiscard]]
consteval ulight_lang_entry make_lang_entry(std::string_view name,
                                            ulight_lang lang) {
...
static_assert(std::ranges::none_of(ulight_lang_display_names,
                                   [](ulight_string_view str) {
                                     return str.length == 0;
                                   }));
...
static_assert(std::ranges::is_sorted(ulight_lang_list, {},
                                     ulight_lang_entry_to_sv));
...
ulight_lang ulight_get_lang(const char *name, size_t name_length) noexcept {
  ...
  const ulight_lang_entry *const result = std::ranges::lower_bound(
      ulight_lang_list, search_value, {}, ulight_lang_entry_to_sv);
...
ULIGHT_EXPORT
ulight_string_view
ulight_highlight_type_long_string(ulight_highlight_type type) noexcept {
...
ulight_highlight_type_long_string_u8(ulight_highlight_type type) noexcept {
  const std::u8string_view result =
      ulight::highlight_type_long_string_u8(ulight::Highlight_Type(type));
...
ULIGHT_EXPORT
ulight_string_view
ulight_highlight_type_short_string(ulight_highlight_type type) noexcept {
...
ULIGHT_EXPORT
ulight_u8string_view
ulight_highlight_type_short_string_u8(ulight_highlight_type type) noexcept {
  const std::u8string_view result =
      ulight::highlight_type_short_string_u8(ulight::Highlight_Type(type));
  return {result.data(), result.size()};
}
...
ULIGHT_EXPORT
ulight_string_view
ulight_highlight_type_id(ulight_highlight_type type) noexcept {
...
ulight_state *ulight_new() noexcept {
  void *const result =
      ulight_alloc(sizeof(ulight_state), alignof(ulight_state));
...
ulight_status error(ulight_state *state, ulight_status status,
...
void check_flush_validity(ulight_state *state,
                          std::span<const ulight_token> tokens) {
...
ulight_status ulight_source_to_tokens(ulight_state *state) noexcept {
  if (state->source == nullptr && state->source_length != 0) {
    return error(state, ULIGHT_STATUS_BAD_STATE,
                 u8"source is null, but source_length is nonzero.");
    return ULIGHT_STATUS_BAD_STATE;
  }
  if (state->token_buffer == nullptr) {
    return error(state, ULIGHT_STATUS_BAD_BUFFER,
                 u8"token_buffer must not be null.");
  }
  if (state->token_buffer_length == 0) {
    return error(state, ULIGHT_STATUS_BAD_BUFFER,
                 u8"token_buffer_length must be nonzero.");
  }
  if (state->flush_tokens == nullptr) {
    return error(state, ULIGHT_STATUS_BAD_BUFFER,
                 u8"flush_tokens must not be null.");
  }
  if (state->lang == ULIGHT_LANG_NONE || int(state->lang) > ULIGHT_LANG_COUNT) {
    return error(state, ULIGHT_STATUS_BAD_LANG,
                 u8"The given language (numeric value) is invalid.");
  }

  ulight::Non_Owning_Buffer<ulight_token> buffer{
      state->token_buffer, state->token_buffer_length, state->flush_tokens_data,
      state->flush_tokens};
  // This may actually lead to undefined behavior.
  // Counterpoint: it works on my machine.
  const std::u8string_view source{
      std::launder(reinterpret_cast<const char8_t *>(state->source)),
      state->source_length};
  ...
  try {
#endif
    const ulight::Status result = ulight::highlight(
        buffer, source, ulight::Lang(state->lang), &memory, options);
    ...
    return error(state, ULIGHT_STATUS_BAD_TEXT,
                 u8"The given source code is not correctly UTF-8-encoded.");
  } catch (const std::bad_alloc &) {
    return error(state, ULIGHT_STATUS_BAD_ALLOC,
                 u8"An attempt to allocate memory during highlighting failed.");
  } catch (...) {
    return error(state, ULIGHT_STATUS_INTERNAL_ERROR,
                 u8"An internal error occurred.");
  }
#endif
}
...
ulight_status ulight_source_to_html(ulight_state *state) noexcept {
  ...
  if (state->text_buffer == nullptr) {
    return error(state, ULIGHT_STATUS_BAD_BUFFER,
                 u8"text_buffer must not be null.");
  }
  if (state->text_buffer_length == 0) {
    return error(state, ULIGHT_STATUS_BAD_BUFFER,
                 u8"text_buffer_length must be nonzero.");
  }
  if (state->flush_text == nullptr) {
    return error(state, ULIGHT_STATUS_BAD_BUFFER,
                 u8"flush_text must not be null.");
  }
  if (state->html_tag_name == nullptr) {
    return error(state, ULIGHT_STATUS_BAD_STATE,
                 u8"html_tag_name must not be null.");
  }
  if (state->html_tag_name_length == 0) {
    return error(state, ULIGHT_STATUS_BAD_STATE,
                 u8"html_tag_name_length must be nonzero.");
  }
  if (state->html_attr_name == nullptr) {
    return error(state, ULIGHT_STATUS_BAD_STATE,
                 u8"html_attr_name must not be null.");
  }
  if (state->html_attr_name_length == 0) {
    return error(state, ULIGHT_STATUS_BAD_STATE,
                 u8"html_attr_name_length must be nonzero.");
  }

  const std::string_view source_string{state->source, state->source_length};
  const std::string_view html_tag_name{state->html_tag_name,
                                       state->html_tag_name_length};
  const std::string_view html_attr_name{state->html_attr_name,
                                        state->html_attr_name_length};

  ulight::Non_Owning_Buffer<char> buffer{
      state->text_buffer, state->text_buffer_length, state->flush_text_data,
      state->flush_text};
  ...
  try {
#endif
    // It is common that the final token doesn't encompass the last code unit in
    // the source. For example, there can be a trailing '\n' at the end of the
    // file, without highlighting.
