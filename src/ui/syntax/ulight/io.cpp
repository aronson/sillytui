...
std::expected<void, IO_Error_Code> file_to_bytes_chunked(
  ...
  do {
    ...
    const std::span<std::byte> chunk{reinterpret_cast<std::byte *>(buffer),
                                     read_size};
    consume_chunk(chunk);
...
std::expected<void, IO_Error_Code> load_utf8_file(std::vector<char8_t> &out,
                                                  std::string_view path) {
  ...
  const std::u8string_view str{out.data() + initial_size,
...
std::expected<std::vector<char8_t>, IO_Error_Code>
load_utf8_file(std::string_view path) {
...
[[nodiscard]]
std::expected<std::vector<char32_t>, IO_Error_Code>
load_utf32le_file(std::string_view path) {
  ...
  // We could make this code more portable in the long run, but I don't care for
  // now.
