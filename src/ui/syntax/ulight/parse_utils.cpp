...
Blank_Line find_blank_line_sequence // NOLINT(bugprone-exception-escape)
  ...
  static_assert(!Blank_Line{},
                "A value-initialized Blank_Line should be falsy");
...
std::optional<unsigned long long> parse_uinteger_digits(std::u8string_view text,
                                                        int base) {
...
Line_Result match_crlf_line(std::u8string_view str) {
  ...
  const std::size_t terminator_length = str[length] == u8'\r' &&
                                                length + 1 < str.length() &&
                                                str[length + 1] == u8'\n'
                                            ? 2
                                            : 1;
...
parse_uinteger_literal(std::u8string_view str) noexcept {
  if (str.empty()) {
    return {};
  }
...
parse_integer_literal(std::u8string_view str) noexcept {
  if (str.empty()) {
    return std::nullopt;
  }
...
std::size_t match_digits(std::u8string_view str, int base) {
  const auto predicate = [base](char8_t c) {
    return is_ascii_digit_base(c, base);
  };
  return ascii::length_if(str, predicate);
}

Digits_Result match_separated_digits(std::u8string_view str, int base,
                                     char8_t separator) {
  if (separator == 0) {
...
Common_Number_Result match_common_number(const std::u8string_view str,
                                         const Common_Number_Options &options) {
  ...
  if (((int(options.signs) & int(Matched_Signs::plus_only)) &&
       str.starts_with(u8'+')) ||
      ((int(options.signs) & int(Matched_Signs::minus_only)) &&
       str.starts_with(u8'-'))) {
    result.sign = 1;
    length += 1;
  }
  const auto base = [&] -> int {
    ...
    return str.substr(length).starts_with(u8'0')
               ? options.default_leading_zero_base
               : options.default_base;
  }();
  {
    const auto [integer_digits, integer_error] = match_separated_digits(
        str.substr(length), base, options.digit_separator);
  ...
  if (allow_float && str.substr(length).starts_with(u8'.')) {
    ...
    const auto [fractional_digits, fractional_error] = match_separated_digits(
        str.substr(length + 1), base, options.digit_separator);
  ...
  ULIGHT_DEBUG_ASSERT((result.sign + result.prefix + result.integer +
                       result.radix_point + result.fractional +
                       result.exponent_sep + result.exponent_digits +
                       result.suffix) == result.length);
  return result;
}
...
Suffix_Number_Result match_suffix_number(
  ...
  const std::size_t length =
      ascii::length_if(str, [s = digit_separator](char8_t c) {
  ...
  const bool erroneous = [&] {
    for (std::size_t i = 0; i + suffix.length < length; ++i) {
      ...
      const bool repeated_separator = digit_separator != 0 &&
                                      c == digit_separator //
                                      && i != 0            //
  ...
  // This covers cases like "zh" or "__h" where "h" is treated as a hexadecimal
  // prefix. "z" would be too large, and "__" would be leading and consecutive
  // digit separators. However, rather than aggressively interpreting these as
  // suffixed numbers, we say that there is no match in this scenario.
  if (erroneous && !is_ascii_digit(number[0])) {
    return {};
  }
  // On the contrary, if we have something like 0__h or 0zh with an "h" suffix
  // for hex numbers, this could not be interpreted as an identifier anyway, so
  // we consider it a match, but erroneous.
