...
bool starts_with_number(std::u8string_view str) {
  ...
  return !str.empty() && //
         (is_ascii_digit(str[0]) ||
          (str.length() > 1 && str[0] == u8'.' && is_ascii_digit(str[1])));
}
...
std::size_t match_number(std::u8string_view str) {
  ...
  consume_digits();
  if (length + 1 < str.length() && str[length] == u8'.' &&
      is_ascii_digit(str[length + 1])) {
    length += 2;
    consume_digits();
  }
  if (length + 1 < str.length() &&
      (str[length] == u8'e' || str[length] == u8'E')) {
    const bool has_sign = str[length + 1] == u8'+' || str[length + 1] == u8'-';
    if (has_sign && length + 2 < str.length() &&
...
std::size_t match_escaped_code_point(std::u8string_view str) {
  ...
  std::size_t length = 0;
  while (length < 6 && length < str.length() &&
         is_ascii_hex_digit(str[length])) {
    ++length;
  }
  if (length != 0) {
...
Ident_Result match_ident_like_token(std::u8string_view str) {
  ...
  // We deviate from the CSS algorithm in the regard that we don't consume the
  // following parenthesis or the following URL, just the identifier.
...
public:
  ...
  bool operator()() {
    while (!remainder.empty()) {
      ...
      case u8'!': {
        ...
        const std::size_t white_length =
            html::match_whitespace(remainder.substr(1));
        if (const std::size_t name_length =
          ...
          if (ascii::starts_with_ignore_case(
            ...
            emit_and_advance(1 + white_length + important.length(),
  ...
  void consume_whitespace() { advance(html::match_whitespace(remainder)); }

  void consume_comments() {
    // https://www.w3.org/TR/css-syntax-3/#consume-comment
    while (const cpp::Comment_Result block_comment =
               cpp::match_block_comment(remainder)) {
  ...
  void consume_string_token(char8_t quote_char) {
    ...
    while (length < remainder.length()) {
      ...
      if (remainder[length] == u8'\\') {
        flush();
        const std::size_t escape_length =
            match_escaped_code_point(remainder.substr(1)) + 1;
...
bool highlight_css(Non_Owning_Buffer<Token> &out, std::u8string_view source,
