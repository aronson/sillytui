...
#define ULIGHT_CPP_TOKEN_TYPE_LENGTH(id, code, highlight, source)              \
  (sizeof(u8##code) - 1),
#define ULIGHT_CPP_TOKEN_HIGHLIGHT_TYPE(id, code, highlight, source)           \
  (Highlight_Type::highlight),
#define ULIGHT_CPP_TOKEN_TYPE_FEATURE_SOURCE(id, code, highlight, source)      \
...
constexpr auto is_cpp_whitespace_lambda = [](char8_t c) {
...
std::size_t match_newline_escape(std::u8string_view str) {
  ...
  // new-line > followed by a new-line character is deleted, > splicing physical
...
std::size_t match_preprocessing_directive(std::u8string_view s, Lang c_or_cpp) {
  ...
  const std::optional<Token_Type> first_token =
      match_preprocessing_op_or_punc(s, c_or_cpp);
  if (first_token != Token_Type::pound &&
      first_token != Token_Type::pound_alt) {
    return {};
  }
...
Comment_Result match_block_comment(std::u8string_view s) {
  ...
  // naive: nesting disallowed, but line comments can be nested in block
  // comments
...
Literal_Match_Result match_integer_literal(std::u8string_view s) {
  ...
  if (s.starts_with(u8"0x")) {
    ...
    return {Literal_Match_Status::ok, digits + 2,
            Integer_Literal_Type::hexadecimal};
  }
  if (s[0] == '0') {
    const std::size_t digits = match_digits(s, 8);
    return {Literal_Match_Status::ok, digits,
            digits == 1 ? Integer_Literal_Type::decimal
                        : Integer_Literal_Type::octal};
  }
  ...
  while (length < str.size()) {
    ...
    case u8'\'': {
      if (length + 1 < str.size() &&
          is_cpp_ascii_identifier_continue(str[length + 1])) {
        length += 2;
        break;
      }
      return length;
    ...
    case u8'P': {
      ...
      // pp-number "P" sign
      if (length + 1 < str.size() &&
          (str[length + 1] == u8'-' || str[length + 1] == u8'+')) {
        length += 2;
      }
    ...
    default: {
      ...
      const auto [code_point, units] =
          utf8::decode_and_length_or_replacement(remainder);
...
Escape_Result match_escape_sequence(std::u8string_view str) {
  constexpr auto with_type = [](ulight::Escape_Result result,
                                Escape_Type type) {
    return Escape_Result{
        .length = result.length, .type = type, .erroneous = result.erroneous};
  };
  ...
  case u8'u': {
    ...
    if (str.length() >= 3 && str[2] == u8'{') {
      return with_type(match_common_escape<Common_Escape::hex_braced>(str, 2),
                       Escape_Type::universal);
    }
    return with_type(match_common_escape<Common_Escape::hex_4>(str, 2),
                     Escape_Type::universal);
  }
  case u8'U': {
    return with_type(match_common_escape<Common_Escape::hex_8>(str, 2),
                     Escape_Type::universal);
  }
  case u8'N': {
  ...
  case u8'x': {
    ...
    if (str.length() >= 3 && str[2] == u8'{') {
      return with_type(match_common_escape<Common_Escape::hex_braced>(str, 2),
                       Escape_Type::hexadecimal);
    }
    return with_type(match_common_escape<Common_Escape::hex_1_to_inf>(str, 2),
                     Escape_Type::hexadecimal);
  }
  case u8'o': {
    // https://eel.is/c++draft/lex.literal#nt:octal-escape-sequence
    if (str.length() >= 3 && str[2] == u8'{') {
      return with_type(match_common_escape<Common_Escape::octal_braced>(str, 2),
                       Escape_Type::octal);
    }
  ...
  case u8'7': {
    // https://eel.is/c++draft/lex.literal#nt:octal-escape-sequence
    return with_type(match_common_escape<Common_Escape::octal_1_to_3>(str, 1),
                     Escape_Type::octal);
  }
...
constexpr bool is_d_char(char8_t c) {
  return is_ascii(c) && !is_cpp_whitespace(c) && c != u8'(' && c != u8')' &&
         c != '\\';
}
...
std::optional<Token_Type> match_preprocessing_op_or_punc(std::u8string_view str,
  ...
  switch (str[0]) {
  case u8'#':
    return str.starts_with(u8"##") ? pound_pound : pound;
  case u8'%':
  ...
  case u8'~':
    return tilde;
  case u8'!':
    return str.starts_with(u8"!=") ? exclamation_eq : exclamation;
  case u8'+':
  ...
  case u8'*':
    return str.starts_with(u8"*=") ? asterisk_eq : asterisk;
  case u8'/':
    return str.starts_with(u8"/=") ? slash_eq : slash;
  case u8'^':
  ...
  case u8'|':
    return str.starts_with(u8"|=")   ? pipe_eq //
           : str.starts_with(u8"||") ? pipe_pipe
                                     : pipe;
  case u8'=':
    return str.starts_with(u8"==") ? eq_eq : eq;
  case u8',':
    return comma;
  default:
    return {};
...
Highlight_Type usual_fallback_highlight(std::u8string_view id) {
  return id.ends_with(u8"_t") ? Highlight_Type::name_type
                              : Highlight_Type::name;
}
...
private:
  ...
  std::size_t index = 0;
  // We need to keep track of whether we're on a "fresh line" for preprocessing
  // directives. A line is fresh if we've not encountered anything but
  // whitespace on it yet.
...
public:
  Highlighter(Non_Owning_Buffer<Token> &out, std::u8string_view source,
              Lang c_or_cpp, const Highlight_Options &options)
      : out{out}, source{source}, c_or_cpp{c_or_cpp}, options{options} {
    ULIGHT_ASSERT(c_or_cpp == Lang::c || c_or_cpp == Lang::cpp);
  }
...
private:
  ...
  void advance(std::size_t length) { index += length; }
...
public:
  ...
  bool expect_line_comment() {
    if (const std::size_t line_comment_length =
            match_line_comment(remainder())) {
  ...
  bool expect_block_comment() {
    if (const Comment_Result block_comment = match_block_comment(remainder())) {
      const std::size_t terminator_length =
          2 * std::size_t(block_comment.is_terminated);
      emit(index, 2, Highlight_Type::comment_delim); // /*
      const std::size_t content_length =
          block_comment.length - 2 - terminator_length;
      if (content_length != 0) {
  ...
  bool expect_character_literal() {
    ...
    if (prefix_length != 0) {
      ...
      const auto prefix_highlight = prefix == u8"u8" || prefix == u8"u" ||
                                            prefix == u8"U" || prefix == u8"L"
  ...
  void consume_char_sequence_and_suffix(char8_t quote_char) {
    ...
    while (index < source.size()) {
      ...
      if (source[index] == u8'\\') {
        if (const Escape_Result escape = match_escape_sequence(remainder())) {
          if (escape.type == Escape_Type::newline) {
            ...
            flush_chars();
            emit_and_advance(escape.length,
                             escape.erroneous ? Highlight_Type::error
      ...
      const auto code_units =
          std::size_t(utf8::sequence_length(source[index], 1));
  ...
  void consume_raw_string_and_suffix() {
    ...
    fresh_line = false;
    const std::size_t d_char_sequence_length =
        match_d_char_sequence(rem.substr(1));
    const std::u8string_view d_char_sequence =
        rem.substr(1, d_char_sequence_length);
    ...
    for (; raw_length < rem.size(); ++raw_length) {
      ...
      emit_and_advance(d_char_sequence_length + 2,
                       Highlight_Type::string_delim);
      consume_string_suffix();
      return;
    }
  ...
  bool expect_identifier_or_keyword(
      Highlight_Type fallback_highlight(std::u8string_view)) {
    ...
    const auto highlight =
        keyword && feature_in_mask(*keyword, feature_source_mask)
  ...
  bool expect_preprocessing_op_or_punc() {
    if (const std::optional<Token_Type> op =
      ...
      const bool possible_directive =
          op == Token_Type::pound || op == Token_Type::pound_alt;
  ...
  bool expect_non_whitespace() {
    if (const std::size_t non_white_length =
            match_non_whitespace(remainder())) {
      // Don't emit any highlighting.
      // To my understanding, this currently only matches backslashes at the end
      // of the line. We don't have a separate phase for these, so whatever,
      // this seems fine.
      fresh_line = false;
...
bool highlight_c( //
    Non_Owning_Buffer<Token> &out, std::u8string_view source,
    std::pmr::memory_resource *, const Highlight_Options &options) {
