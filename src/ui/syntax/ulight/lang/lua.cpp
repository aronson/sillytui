...
#define ULIGHT_LUA_TOKEN_TYPE_LENGTH(id, code, highlight, strict)              \
  (sizeof(u8##code) - 1),
#define ULIGHT_LUA_TOKEN_HIGHLIGHT_TYPE(id, code, highlight, strict)           \
...
inline constexpr std::bitset<lua_token_type_count> token_type_strictness{
    ULIGHT_LUA_TOKEN_ENUM_DATA(ULIGHT_LUA_TOKEN_TYPE_STRICT_STRING),
    lua_token_type_count};
...
[[nodiscard]]
std::optional<Lua_Token_Type>
lua_token_type_by_code(std::u8string_view code) noexcept {
...
constexpr auto is_lua_whitespace_lambda = [](char8_t c) {
  return is_lua_whitespace(c);
};
...
std::size_t match_line_comment(std::u8string_view s) noexcept {
  ...
  if (s.length() >= 3 && s[2] == u8'[') {
    ...
    // If we have a well-formed opening [, it's a block comment, not a line
    // comment.
    ...
    while (closing_idx < s.length()) {
      ...
      if (s[closing_idx] == u8']') {
        ...
        for (std::size_t i = 0; i < eq_count; i++) {
          if (closing_idx + 1 + i >= s.length() ||
              s[closing_idx + 1 + i] != u8'=') {
            found = false;
            break;
          }
        }
    ...
    while (length < str.length()) {
      ...
      if (escaped) {
        ...
        return String_Literal_Result{
            .length = length + 1, .is_long_string = false, .terminated = true};
      } else if (c == u8'\n') {
        // Unterminated.
        return String_Literal_Result{
            .length = length, .is_long_string = false, .terminated = false};
      }
  ...
  if (str.starts_with(u8"[[")) {
    ...
    while (length < str.length() - 1) {
      if (str[length] == u8']' && str[length + 1] == u8']') {
        return String_Literal_Result{
            .length = length + 2, .is_long_string = true, .terminated = true};
      }
      length++;
    }
    ...
    return String_Literal_Result{
        .length = str.length(), .is_long_string = true, .terminated = false};
  }
  ...
  if (str.starts_with(u8"[") && str.length() >= 3) {
    ...
    while (closing_idx < str.length()) {
      ...
      if (str[closing_idx] == u8']') {
        bool found = true;
        for (std::size_t i = 0; i < eq_count; i++) {
          if (closing_idx + 1 + i >= str.length() ||
              str[closing_idx + 1 + i] != u8'=') {
            found = false;
            break;
          }
        }
...
std::size_t match_number(std::u8string_view str) {
  ...
  if (str.length() >= 2 && str[0] == u8'0' &&
    ...
    if (length < str.length() &&
        (str[length] == u8'p' || str[length] == u8'P')) {
      length++;

      // Optional sign.
      if (length < str.length() &&
          (str[length] == u8'+' || str[length] == u8'-')) {
        length++;
      }
  ...
  if (length == 0 && str.length() >= 2 && str[0] == u8'.' &&
...
std::size_t match_identifier(std::u8string_view str) {
  ...
  while (length < str.length()) {
    const auto [code_point, units] =
        utf8::decode_and_length_or_replacement(str.substr(length));
  ...
  case u8'<':
    if (str.length() > 1) {
      ...
      if (str.length() >= 6 && str.substr(1, 5) == u8"const>") {
        return {}; // Return empty for <const> so it's handled by the special
                   // case
      }
    }
    return less;
...
bool highlight_lua(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                   std::pmr::memory_resource *,
                   const Highlight_Options &options) {
  const auto emit = [&](std::size_t begin, std::size_t length,
                        Highlight_Type type) {
  ...
  while (index < source.size()) {
    ...
    // Line comments - Lua treats each line separately, no backslash
    // continuation
    if (const std::size_t line_comment_length =
            lua::match_line_comment(remainder)) {
    ...
    // Block comments.
    if (const lua::Comment_Result block_comment =
            lua::match_block_comment(remainder)) {
      // Prefix --[[ or --[=[.
      std::size_t prefix_length = 4; // Default for --[[.
      if (remainder[2] == u8'[' && remainder[3] != u8'[') {
        prefix_length = 3;
        while (prefix_length < remainder.length() &&
               remainder[prefix_length] == u8'=') {
          prefix_length++;
        }
        prefix_length++; // Handles the [ after equals.
      }

      const std::size_t closing_delim_length =
          block_comment.is_terminated
              ? (remainder[2] == u8'[' && remainder[3] != u8'['
                     ? 2 + (prefix_length - 4)
                     : 2)
              : 0;

      emit(index, prefix_length, Highlight_Type::comment_delim);
      emit(index + prefix_length,
           block_comment.length - prefix_length - closing_delim_length,
           Highlight_Type::comment);

      if (block_comment.is_terminated) {
        emit(index + block_comment.length - closing_delim_length,
             closing_delim_length, Highlight_Type::comment_delim);
      }
    ...
    // String literals.
    if (const lua::String_Literal_Result string =
            lua::match_string_literal(remainder)) {
      if (string.is_long_string) {
        // [[ ]] or [=[ ]=] multi-line strings, highlight the delimiters
        // separately.
        std::size_t opening_delim_len = 2;
        std::size_t closing_delim_len = string.terminated ? 2 : 0;

        // [=[ style with equals signs.
        if (remainder[0] == u8'[' && remainder.length() >= 3 &&
            remainder[1] == u8'=') {
          opening_delim_len = 1;
        ...
        emit(index + opening_delim_len,
             string.length - opening_delim_len - closing_delim_len,
             Highlight_Type::string);

        if (string.terminated) {
          emit(index + string.length - closing_delim_len, closing_delim_len,
               Highlight_Type::string);
        }
      } else {
    ...
    if (const std::size_t id_length = lua::match_identifier(remainder)) {
      ...
      const auto highlight = keyword ? lua::lua_token_type_highlight(*keyword)
                                     : Highlight_Type::name;
