...
#define ULIGHT_RUST_TOKEN_TYPE_LENGTH(id, code, highlight)                     \
  (sizeof(u8##code) - 1),
#define ULIGHT_RUST_TOKEN_HIGHLIGHT_TYPE(id, code, highlight)                  \
...
constexpr bool is_illegal_raw_identifier(std::u8string_view id) {
  ...
  return id == u8"crate"sv || id == u8"self"sv || id == u8"super"sv ||
         id == u8"Self"sv || id == u8"_"sv;
}
...
[[nodiscard]]
std::optional<String_Classify_Result>
classify_string_prefix(const std::u8string_view str) {
  if (str.empty()) {
    return {};
  }
  ...
  const auto *const result = std::ranges::find_if(
      prefixes,
      [&](std::u8string_view sequence) { return str.starts_with(sequence); },
...
Common_Number_Result match_number(const std::u8string_view str) {
  ...
  static constexpr auto match_suffix =
      [](std::u8string_view str) -> std::size_t {
...
Escape_Result match_escape_sequence(const std::u8string_view str,
  ...
  // see
  // https://doc.rust-lang.org/reference/grammar.html#grammar-summary-UNICODE_ESCAPE
  case u8'u':
...
std::optional<Token_Type> match_punctuation(std::u8string_view str) noexcept {
  ...
  case u8'%':
    return str.starts_with(u8"%=") ? percent_eq : percent;
  case u8'&':
  ...
  case u8'+':
    return str.starts_with(u8"+=") ? plus_eq : plus;
  case u8',':
    return comma;
  case u8'-':
  ...
  case u8':':
    return str.starts_with(u8"::") ? colon_colon : colon;
  case u8';':
    return semicolon;
  case u8'<':
  ...
  case u8']':
    return right_square;
  case u8'^':
    return str.starts_with(u8"^=") ? caret_eq : caret;
  case u8'{':
    return left_brace;
  case u8'|':
  ...
  bool operator()() {
    while (!eof()) {
      ...
      if (!expect_token_or_comment()) {
        const auto [_, error_length] =
            utf8::decode_and_length_or_replacement(remainder);
        ULIGHT_ASSERT(error_length != 0);
        emit_and_advance(std::size_t(error_length), Highlight_Type::error,
                         Coalescing::forced);
      }
    }
    return true;
...
private:
  void consume_whitespace() {
    const std::size_t space = utf8::length_if(
        remainder, [](char32_t c) { return is_rust_whitespace(c); });
    advance(space);
  }
  ...
  bool expect_block_comment() {
    if (const Comment_Result block_comment = match_block_comment(remainder)) {
      ...
      const std::size_t content_length =
          block_comment.length - 2 - suffix_length;
      if (content_length != 0) {
  ...
  bool expect_raw_identifier() {
    ...
    const std::size_t id_length =
        match_identifier(remainder.substr(prefix.length()));
    if (!id_length) {
      return false;
    }
    ...
    } else if (remainder.substr(prefix.length() + id_length)
                   .starts_with(u8'!')) {
  ...
  bool expect_char_or_byte_literal() {
    ...
    //
    // In other languages, we usually handle char and string literals in the
    // same function. However, there are too many nuances, such as nested
    // newlines being permitted. Also, we have to be quite pessimistic with char
    // literals. If there is no closing quote, it should be a LIFETIME_TOKEN.
    ...
    case u8'\\': {
      const Escape_Result escape = match_escape_sequence(
          remainder.substr(content_start), String_Type::string);
      if (!escape) {
        return false;
      }
      if (escape.length == 2 && remainder[content_start + 1] == u8'\n') {
        // Handle the case of a STRING_CONTINUE in a CHAR_LITERAL,
        // which we may have matched with match_escape_sequence, but is not
        // allowed.
        return false;
      }
      if (!remainder.substr(content_start + escape.length)
               .starts_with(u8'\'')) {
        return false;
      }
      emit_result(escape.length, escape.erroneous
    ...
    default: {
      const auto [_, char_length] = utf8::decode_and_length_or_replacement(
          remainder.substr(content_start));
      if (!remainder.substr(content_start + std::size_t(char_length))
               .starts_with(u8'\'')) {
        return false;
      }
  ...
  bool expect_string_literal() {
    ...
    const std::size_t raw_hash_length =
        string_type_is_raw(string_type)
            ? ascii::length_before_not(remainder.substr(prefix_length), u8'#')
            : 0uz;

    if (const auto after_prefix =
            remainder.substr(prefix_length + raw_hash_length);
  ...
  void consume_string_content_and_suffix(String_Type type,
                                         const std::size_t raw_hash_length) {
    // https://doc.rust-lang.org/reference/tokens.html#character-and-string-literals
      ...
      while (length < remainder.length()) {
        if (remainder[length] == terminator) {
          ...
          if (terminating_hashes >= raw_hash_length) {
            flush();
            emit_and_advance(1 + terminating_hashes,
      ...
      while (length < remainder.length()) {
        ...
        if (remainder[length] == u8'\\') {
          ...
          if (const Escape_Result escape =
                  match_escape_sequence(remainder, type)) {
    ...
    // character and string literals technically have an optional SUFFIX
    // (lexically), but this is not used in the language.
    if (const std::size_t suffix = match_identifier(remainder)) {
      emit_and_advance(suffix, Highlight_Type::error);
    }
  }
  ...
  bool expect_lifetime_token() {
    ...
    const auto emit_result = [&](std::size_t delim_length,
                                 std::size_t id_length) {
      const bool is_label =
          remainder.substr(delim_length + id_length).starts_with(u8':');
      emit_and_advance(delim_length, is_label
                                         ? Highlight_Type::name_label_delim
                                         : Highlight_Type::name_lifetime_delim);
      emit_and_advance(id_length, is_label ? Highlight_Type::name_label
                                           : Highlight_Type::name_lifetime);
    };

    if (remainder.starts_with(raw_prefix)) {
      const std::size_t id_length =
          match_identifier(remainder.substr(raw_prefix.length()));
      if (!id_length) {
        return false;
      }
      const std::u8string_view id =
          remainder.substr(raw_prefix.length(), id_length);
      if (is_illegal_raw_identifier(id)) {
        emit_and_advance(raw_prefix.length() + id_length,
                         Highlight_Type::error);
      } else {
        emit_result(raw_prefix.length(), id_length);
      }
      // There should not be a closing apostrophe here because any such case
      // would have been interpreted as a character literal already.
  ...
  bool expect_punctuation() {
    ...
    if (const std::optional<Token_Type> symbol = match_punctuation(remainder)) {
      emit_and_advance(token_type_length(*symbol),
                       token_type_highlight(*symbol));
      return true;
    }
    return false;
  }
...
bool highlight_rust(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                    std::pmr::memory_resource *memory,
                    const Highlight_Options &options) {
