  ...
  bool operator()() {
    ...
    while (!eof()) {
      ...
      case u8'-': {
        ...
        // it is common among users to use '-' as part of identifiers in the
        // grammar instead of subtraction. We also support that as long as '-'
        // is directly preceded by another identifier character.
...
private:
  void consume_delimited(std::u8string_view open, std::u8string_view close,
                         Highlight_Type open_type, Highlight_Type content_type,
  ...
  void consume_comment() {
    consume_delimited(u8"(*", u8"*)", //
                      Highlight_Type::comment_delim, Highlight_Type::comment,
                      Highlight_Type::comment_delim);
  }

  void consume_string(char8_t quote_char) {
    const std::u8string_view quote_string{&quote_char, 1};
    consume_delimited(quote_string, quote_string, //
                      Highlight_Type::string_delim, Highlight_Type::string,
...
bool highlight_ebnf(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                    std::pmr::memory_resource *memory,
                    const Highlight_Options &options) {
