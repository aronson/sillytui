...
std::size_t match_text(std::u8string_view str) {
  const std::size_t result = ascii::length_if_not(
      str, [](char8_t c) { return c == u8'<' || c == u8'&'; });
...
struct XML_Highlighter : Highlighter_Base {
  ...
  XML_Highlighter(Non_Owning_Buffer<Token> &out, std::u8string_view source,
...
private:
  bool expect_processing_instruction() {
    ...
    // TODO: this check for whether xml is in the name is most likely
  ...
  bool expect_cdata_section() {
    if (const auto cdata_section = html::match_cdata(remainder)) {
      std::size_t pure_cdata_len =
          cdata_section.length - cdata_section_prefix.length();

      if (cdata_section.terminated) {
        pure_cdata_len -= cdata_section_suffix.length();
      }

      emit_and_advance(cdata_section_prefix.length(),
                       Highlight_Type::name_macro);

      advance(pure_cdata_len);

      if (cdata_section.terminated) {
        emit_and_advance(cdata_section_suffix.length(),
                         Highlight_Type::name_macro);
      }
  ...
  bool expect_start_tag() {
    ...
    const std::size_t name_length =
        expect_name(Highlight_Type::markup_tag, is_tag_name_end);
  ...
  std::size_t expect_name(Highlight_Type type, Stop is_stop) {
    ...
    while (piece_length < remainder.length() &&
           !is_stop(remainder.substr(piece_length))) {
      const auto [code_point, length] = utf8::decode_and_length_or_replacement(
          remainder.substr(piece_length));
      if ((total_length == 0 && !is_xml_name_start(code_point)) ||
          !is_xml_name(code_point)) {
        if (piece_length) {
...
bool highlight_xml(Non_Owning_Buffer<Token> &out, std::u8string_view source,
