...
std::optional<Token_Type> match_operator(std::u8string_view str) {
  ...
  case u8'@':
    return at;
  case u8'[':
    return str.starts_with(u8"[[") ? left_square_square : left_square;
  case u8']':
    return str.starts_with(u8"]]") ? right_square_square : right_square;
  case u8'{':
    return left_brace;
  case u8'|':
    return str.starts_with(u8"||") ? pipe_pipe : pipe;
  case u8'}':
    return right_brace;
  case u8'~':
    return tilde;
...
private:
  ...
  void consume_word(Context context) {
    ...
    case State::before_argument: {
      const auto highlight = remainder.starts_with(u8'-')
                                 ? Highlight_Type::name_shell_option
  ...
  void consume_escape_character() {
    if (remainder.starts_with(u8"\\\n")) {
      ...
      emit_and_advance(std::min(2uz, remainder.length()),
...
bool highlight_bash(Non_Owning_Buffer<Token> &out, std::u8string_view source,
