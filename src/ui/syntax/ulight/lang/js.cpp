...
#define ULIGHT_JS_TOKEN_TYPE_LENGTH(id, code, highlight, source)               \
  (sizeof(u8##code) - 1),
#define ULIGHT_JS_TOKEN_HIGHLIGHT_TYPE(id, code, highlight, source)            \
  (Highlight_Type::highlight),
#define ULIGHT_JS_TOKEN_TYPE_FEATURE_SOURCE(id, code, highlight, source)       \
...
constexpr bool token_type_is_available(Token_Type type, Mode mode) {
  ...
  case Mode::javascript:
    return Underlying(source) & Underlying(Feature_Source::js);
  case Mode::typescript:
    return Underlying(source) & Underlying(Feature_Source::ts);
  }
...
token_type_by_code(std::u8string_view code) {
  const std::u8string_view *const p =
      std::ranges::lower_bound(token_type_codes, code);
...
constexpr bool token_type_cannot_precede_regex(Token_Type type) {
  // TODO: investigate what the proper set here is, or whether it even makes
  // sense for us
...
Escape_Result match_escape_sequence(const std::u8string_view str) {
  ...
  case u8'3': {
    ...
    if (str.length() >= 3 && is_ascii_octal_digit(str[2])) {
      ...
      const std::size_t length =
          str.length() >= 4 && is_ascii_octal_digit(str[3]) ? 4 : 3;
  ...
  case u8'7': {
    ...
    const std::size_t length =
        str.length() >= 3 && is_ascii_octal_digit(str[2]) ? 3 : 2;
    return {.length = length};
  }
  default: {
...
std::size_t match_line_continuation(std::u8string_view str) {
  ...
  if (const std::size_t terminator =
          match_line_terminator_sequence(str.substr(1))) {
    return terminator + 1;
  }
  return 0;
}
...
std::size_t match_name(std::u8string_view str, Name_Type type) {
  ...
  const auto [first_char, first_units] =
      utf8::decode_and_length_or_replacement(str);
  ...
  const auto is_part = [&](char32_t c) {
    ...
    switch (type) {
    case Name_Type::identifier:
      return false;
    case Name_Type::jsx_identifier:
      return c == U'-';
    case Name_Type::jsx_attribute_name:
      return c == U'-' || c == U':';
    case Name_Type::jsx_element_name:
      return c == U'-' || c == U':' || c == U'.';
    }
...
std::size_t match_regex_flags(std::u8string_view str) {
  ...
  constexpr auto predicate = [](char32_t c) {
    return is_js_identifier_part(c);
  };
...
struct Counting_WSC_Consumer : virtual Whitespace_Comment_Consumer {
  ...
  void whitespace(std::size_t str) final { length += str; }
  void block_comment(Comment_Result comment) final { length += comment.length; }
  void line_comment(std::size_t comment) final { length += comment; }
};

void match_whitespace_comment_sequence(Whitespace_Comment_Consumer &out,
...
JSX_Braced_Result match_jsx_braced(std::u8string_view str) {
  ...
  while (length < str.length()) {
    if (const std::size_t skip_length =
            match_whitespace_comment_sequence(str.substr(length))) {
      length += skip_length;
    }
...
struct Counting_JSX_Tag_Consumer final : JSX_Tag_Consumer,
                                         Counting_WSC_Consumer {
  JSX_Type type{};

  void done(JSX_Type t) final { type = t; }
  void advance(std::size_t amount) final { length += amount; }
  void opening_symbol() final { ++length; }
  void element_name(std::size_t name) final { length += name; }
  void attribute_name(std::size_t name) final { length += name; }
  void closing_symbol() final { ++length; }
  void attribute_equals() final { ++length; }
  void string_literal(String_Literal_Result r) final { length += r.length; }
  void braced(JSX_Braced_Result braced) final { length += braced.length; }
};
...
struct Matching_JSX_Tag_Consumer final : JSX_Tag_Consumer {
  ...
  void done(JSX_Type type) final { out.done(type); }
...
bool match_jsx_tag_impl(JSX_Tag_Consumer &consumer, std::u8string_view str,
...
match_operator_or_punctuation(std::u8string_view str) {
  using enum Token_Type;
  ...
  case u8'!':
    return str.starts_with(u8"!==")  ? strict_not_equals
           : str.starts_with(u8"!=") ? not_equals
                                     : logical_not;
  case u8'%':
    return str.starts_with(u8"%=") ? modulo_equal : modulo;
  case u8'&':
  ...
  case u8'*':
    return str.starts_with(u8"**=")  ? exponentiation_equal
           ...
           : str.starts_with(u8"*=") ? multiply_equal
                                     : multiply;
  case u8'+':
    return str.starts_with(u8"++")   ? increment
           : str.starts_with(u8"+=") ? plus_equal
                                     : plus;
  case u8',':
    return comma;
  case u8'-':
    return str.starts_with(u8"--")   ? decrement
           : str.starts_with(u8"-=") ? minus_equal
                                     : minus;
  case u8'.':
    return str.starts_with(u8"...") ? ellipsis : dot;
  case u8'/':
  ...
  case u8'^':
    return str.starts_with(u8"^=") ? bitwise_xor_equal : bitwise_xor;
  case u8'{':
    return left_brace;
  case u8'|':
...
[[nodiscard]]
JSX_Tag_Result match_jsx_tag_impl(std::u8string_view str,
                                  JSX_Tag_Subset subset = JSX_Tag_Subset::all) {
...
/// A lot of that has to do with avoiding recursion when parsing the contents of
/// template literals, and some of it has to do with allowing
/// RegularExpressionLiteral and Hashbang only in some contexts.
///
/// `hashbang_or_regex` is only used at the start of the file.
/// From that point on, the decision is based on whether a regex literal can
/// appear in the context-free grammar.
...
public:
  Highlighter(Non_Owning_Buffer<Token> &out, std::u8string_view source,
              const Highlight_Options &options, Mode mode)
      : Highlighter_Base{out, source, options}, mode{mode} {}
...
private:
  ...
  /// This is used both for matching braced JS code in JSX, like in `<div
  /// id={get_id()}>`, and for template literals in regular JS.
  ///
  ...
  bool expect_jsx_in_js() {
    ...
    // Only once we've successfully parsed a tag, we consider it to be a JSX
    // tag. Otherwise, we fall back onto regular JS semantics, and consider "<"
    // to be the less-than operator instead.
    //
    // Furthermore, we ignore closing tags at the beginning.

    const JSX_Tag_Result opening =
        match_jsx_tag_impl(remainder, JSX_Tag_Subset::non_closing);
    if (!opening) {
      return false;
    }
    consume_jsx_tag();
    if (opening.type != JSX_Type::self_closing) {
      const bool is_opening = opening.type == JSX_Type::opening ||
                              opening.type == JSX_Type::fragment_opening;
  ...
  void consume_jsx_tag() {
    struct Highlighter_As_Consumer : JSX_Tag_Consumer {
      Highlighter &self;
      Highlighter_As_Consumer(Highlighter &self) : self{self} {}

      void done(JSX_Type) final {}
      void whitespace(std::size_t w) final { self.advance(w); }
      void block_comment(Comment_Result comment) final {
        self.highlight_block_comment(comment);
      }
      void line_comment(std::size_t l) final { self.highlight_line_comment(l); }
      void advance(std::size_t amount) final { self.advance(amount); }
      void opening_symbol() final {
  ...
  void consume_jsx_children_and_closing_tag() {
    ...
    while (!rem.empty()) {
      ...
      case u8'<': {
        ...
        if (tag.type == JSX_Type::opening ||
            tag.type == JSX_Type::fragment_opening) {
          ++depth;
        }
        if (tag.type == JSX_Type::closing ||
            tag.type == JSX_Type::fragment_closing) {
          if (--depth < 0) {
            return;
          }
        }
    ...
    // This isn't really valid code, but it doesn't matter for syntax
    // highlighting.
  }
  ...
  void highlight_jsx_braced(const JSX_Braced_Result &braced) {
    ...
    emit_and_advance(1, Highlight_Type::symbol_brace);
    const std::size_t js_length =
        braced.length - (braced.is_terminated ? 2 : 1);
  ...
  void highlight_block_comment(const Comment_Result &block_comment) {
    ...
    if (block_comment.is_terminated) {
      emit(index + block_comment.length - 2, 2,
  ...
  bool expect_regex() {
    ...
    if (!rem.starts_with(u8'/') || rem.starts_with(u8"/*") ||
        rem.starts_with(u8"//")) {
      return false;
    }
    ...
    for (std::size_t size = 0; size < rem.length(); ++size) {
      ...
      if (escaped) {
        ...
        emit_and_advance(1, Highlight_Type::string_delim);
        if (const std::size_t regex_flags =
                match_regex_flags(rem.substr(size + 1))) {
  ...
  bool expect_private_identifier() {
    if (const std::size_t private_id_length =
            match_private_identifier(remainder)) {
  ...
  bool expect_identifier() {
    ...
    input_element = token_type_is_expr_keyword(*keyword) ? Input_Element::regex
                                                         : Input_Element::div;
  ...
  bool expect_operator_or_punctuation() {
    const std::optional<Token_Type> op =
        match_operator_or_punctuation(remainder);
    ...
    input_element = token_type_cannot_precede_regex(*op) ? Input_Element::div
                                                         : Input_Element::regex;
...
bool highlight_javascript(Non_Owning_Buffer<Token> &out,
...
bool highlight_typescript(Non_Owning_Buffer<Token> &out,
                          std::u8string_view source,
                          std::pmr::memory_resource *,
                          const Highlight_Options &options) {
