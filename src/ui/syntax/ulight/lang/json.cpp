...
Identifier_Result match_identifier(std::u8string_view str) {
  ...
  const std::size_t length = ascii::length_if(
      str, [](char8_t c) { return is_ascii_alphanumeric(c); }, 1);
...
Escape_Result match_escape_sequence(std::u8string_view str,
  ...
  // "\", "u", hex, hex, hex, hex
  const auto [length, erroneous] =
      match_common_escape<Common_Escape::hex_4>(str, 2);
  if (erroneous) {
...
Number_Result match_number(std::u8string_view str) {
  ...
  // JSON doesn't allow leading zeroes except immediately prior to the radix
  // point. However, we still know what was meant by say, "0123".
  ...
  // This guarantees that a non-matching `Number_Result` is equal to a
...
public:
  Highlighter(Non_Owning_Buffer<Token> &out, std::u8string_view source,
              ...
              const Highlight_Options &options, Comment_Policy comments)
      : Highlighter_Base{out, source, memory, options},
        has_comments{comments == Comment_Policy::always_allow ||
                     !options.strict} {}
...
private:
  ...
  bool expect_string(String_Type type) {
    ...
    const auto highlight = type == String_Type::property
                               ? Highlight_Type::markup_attr
    ...
    while (length < remainder.length()) {
      ...
      case u8'\r':
      case u8'\v': {
        // Line breaks are not technically allowed in strings, but we still have
        // to handle them somehow. We do this by considering them to be the end
        // of the string, rather than continuing onto the next line.
        flush();
        return true;
      }
      case u8'\\': {
...
public:
  ...
  Parser(JSON_Visitor &out, std::u8string_view source, JSON_Options options)
      : out{out}, source_length{source.length()}, options{options},
        remainder{source} {}

  [[nodiscard]]
  bool operator()() {
    return consume_whitespace_comments() && consume_value() &&
           consume_whitespace_comments();
  }

private:
  void error(JSON_Error error) { out.error(pos, error); }
  ...
  bool consume_block_comment() {
    if (const js::Comment_Result block_comment =
            match_block_comment(remainder)) {
      if (!block_comment.is_terminated) {
  ...
  bool consume_string(String_Type type) {
    if (!remainder.starts_with(u8'"')) {
      ULIGHT_DEBUG_ASSERT_UNREACHABLE(
          u8"Should have checked for quotes already.");
      return false;
    }
  ...
  [[nodiscard]]
  bool consume_escape() {
    const auto policy = options.escapes == Escape_Parsing::none
                            ? Escape_Policy::match_only
                            : Escape_Policy::parse;
  ...
  bool consume_number() {
    ...
    if (options.parse_numbers) {
      const auto *const str_begin =
          reinterpret_cast<const char *>(number_string.data());
  ...
  bool consume_object() {
    if (!remainder.starts_with(u8'{')) {
      ULIGHT_DEBUG_ASSERT_UNREACHABLE(
          u8"This should have been tested outside.");
      error(JSON_Error::error);
      return false;
    }
    out.push_object(pos);
...
bool highlight_json(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                    std::pmr::memory_resource *memory,
...
bool highlight_jsonc(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                     std::pmr::memory_resource *memory,
                     const Highlight_Options &options) {
  return json::Highlighter{out, source, memory, options,
                           json::Comment_Policy::always_allow}();
}

bool parse_json(JSON_Visitor &visitor, std::u8string_view source,
                JSON_Options options) {
  return json::Parser{visitor, source, options}();
}

bool parse_json(JSON_Visitor &visitor, std::string_view source,
                JSON_Options options) {
  const std::u8string_view u8source{
      reinterpret_cast<const char8_t *>(source.data()), source.length()};
