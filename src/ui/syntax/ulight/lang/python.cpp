...
#define ULIGHT_PYTHON_TOKEN_TYPE_LENGTH(id, code, highlight)                   \
  (sizeof(u8##code) - 1),
#define ULIGHT_PYTHON_TOKEN_HIGHLIGHT_TYPE(id, code, highlight)                \
...
std::optional<String_Prefix> classify_string_prefix(std::u8string_view str) {
  ...
  const auto *const result =
      std::ranges::lower_bound(prefixes, str, {}, &Prefix_And_Type::id);
...
std::optional<Token_Type> match_symbol(std::u8string_view str) noexcept {
  ...
  switch (str[0]) {
  case u8'!':
    return str.starts_with(u8"!=") ? exclamation_eq
                                   : std::optional<Token_Type>{};
  case u8'%':
    return str.starts_with(u8"%=") ? percent_eq : percent;
  case u8'&':
    return str.starts_with(u8"&=") ? amp_eq : amp;
  case u8'(':
    return left_parens;
  case u8')':
    return right_parens;
  ...
  case u8'/':
    return str.starts_with(u8"//=")  ? slash_slash_eq
           ...
           : str.starts_with(u8"/=") ? slash_eq
                                     : slash;
  case u8':':
    return str.starts_with(u8":=") ? colon_eq : colon;
  case u8';':
    return semicolon;
  case u8'<':
  ...
  case u8'>':
    return str.starts_with(u8">>=")   ? greater_greater_eq
           ...
           : str.starts_with(u8">>")  ? greater_greater
                                      : greater;
  case u8'@':
    return str.starts_with(u8"@=") ? at_eq : at;
  case u8'[':
    return left_square;
  case u8'\\':
    return backslash;
  ...
  case u8'{':
    return left_brace;
  case u8'|':
    return str.starts_with(u8"|=") ? pipe_eq : pipe;
  case u8'}':
    return right_brace;
  case u8'~':
    return tilde;
...
struct Highlighter : Highlighter_Base {
  ...
  bool operator()() {
    while (!eof()) {
      ...
      // string prefixes like in r"awoo" are parsed as separate identifier
      // tokens.
...
private:
  ...
  bool expect_comment() {
    ...
    if (const std::size_t length = ascii::length_if_not(
            remainder, [](char8_t c) { return is_python_newline(c); })) {
  ...
  bool expect_string_literal() {
    ...
    const auto effective_prefix = [&] -> String_Prefix {
      ...
      if (const std::optional<String_Prefix> prefix =
              classify_string_prefix(prefix_str)) {
        return *prefix;
      }
    ...
    if (prefix_length != 0) {
      emit_and_advance(prefix_length, prefix_erroneous
    ...
    while (length < remainder.length()) {
      ...
      if (remainder[length] == u8'\\') {
        ...
        if (string_prefix_is_raw(prefix)) {
          ...
          const auto [_, length] =
              utf8::decode_and_length_or_replacement(escaped_units);
          ...
          // so we decode the entire code point and make it an error if it's
          // non-ASCII.
          ...
          emit_and_advance(1 + std::size_t(length), highlight);
                       ...
                       match_escape_sequence(remainder)) {
          emit_and_advance(escape.length, escape.erroneous
                                              ? Highlight_Type::error
                                              : Highlight_Type::string_escape);
        } else {
...
bool highlight_python(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                      std::pmr::memory_resource *memory,
                      const Highlight_Options &options) {
