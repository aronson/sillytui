...
bool is_character_reference_content(std::u8string_view str) {
  if (str.starts_with(u8"#x")) {
    return str.length() > 2 &&
           std::ranges::all_of(str.substr(2),
                               [](char8_t c) { return is_ascii_hex_digit(c); });
  }
  if (str.starts_with(u8'#')) {
    return str.length() > 1 &&
           std::ranges::all_of(str.substr(1),
                               [](char8_t c) { return is_ascii_digit(c); });
  }
  return !str.empty() && std::ranges::all_of(str, [](char8_t c) {
...
std::size_t match_raw_text(std::u8string_view str,
                           std::u8string_view closing_name) {
  ...
  while (!str.empty()) {
    ...
    //   3. Whitespace, ">", or "/".
    //
    // We're not sure if we'll get all three, so we use new_length as a
    // temporary length and only commit to updating length once all parts are
    // matched.
...
Raw_Text_Result
match_escapable_raw_text_piece(std::u8string_view str,
                               std::u8string_view closing_name) {
  // https://html.spec.whatwg.org/dev/syntax.html#cdata-rcdata-restrictions
  std::size_t length = 0;
...
End_Tag_Result match_end_tag_permissive(const std::u8string_view str) {
  ...
  const std::size_t name_end_pos = ascii::find_if(
      str, [](char8_t c) static { return is_html_whitespace(c) || c == u8'>'; },
      2);
...
private:
  ...
  bool expect_cdata() {
    if (const Match_Result cdata = match_cdata(remainder)) {
      ...
      if (cdata.terminated) {
        emit(index + cdata.length - cdata_suffix.length(),
             cdata_suffix.length(), Highlight_Type::name_macro);
      }
  ...
  bool expect_comment() {
    ...
    if (comment.terminated) {
      if (comment.length > comment_suffix.length()) {
        emit_and_advance(comment.length - comment_suffix.length(),
  ...
  bool expect_start_tag_permissive() {
    ...
    if (ascii::equals_ignore_case(name, textarea_tag) ||
      ...
      while (const Raw_Text_Result result =
                 match_escapable_raw_text_piece(remainder, name)) {
  ...
  bool expect_attribute() {
    ...
    // Always returns true because unquoted (possibly zero-length) is always
    // matched.
...
bool highlight_html( //
    ...
    std::pmr::memory_resource *memory, const Highlight_Options &options) {
