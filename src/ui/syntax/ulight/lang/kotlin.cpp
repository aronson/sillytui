...
#define ULIGHT_KOTLIN_TOKEN_TYPE_LENGTH(id, code, highlight)                   \
  (sizeof(u8##code) - 1),
#define ULIGHT_KOTLIN_TOKEN_HIGHLIGHT_TYPE(id, code, highlight)                \
...
Common_Number_Result match_number(std::u8string_view str) {
  ...
  result.erroneous |=
      (result.fractional || result.radix_point) && result.prefix;
  return result;
}
...
std::optional<Token_Type> match_symbol(std::u8string_view str) noexcept {
  ...
  case u8'!':
    return str.starts_with(u8"!=")    ? excl_eq
           ...
           : str.starts_with(u8"!is") ? not_is
                                      : excl;
  case u8'"':
    return str.starts_with(u8"\"\"\"") ? triple_quote_open : quote_open;
  case u8'#':
    return hash;
  case u8'%':
    return str.starts_with(u8"%=") ? mod_assignment : mod;
  case u8'&':
    return str.starts_with(u8"&&") ? conj : none;
  case u8'(':
    return lparen;
  case u8')':
    return rparen;
  case u8'*':
    return str.starts_with(u8"*=") ? mult_assignment : mult;
  case u8'+':
    return str.starts_with(u8"++")   ? incr
           : str.starts_with(u8"+=") ? add_assignment
                                     : add;
  case u8',':
    return comma;
  case u8'-':
    return str.starts_with(u8"--")   ? decr
           ...
           : str.starts_with(u8"->") ? arrow
                                     : sub;
  case u8'.':
    return str.starts_with(u8"...")  ? reserved
           : str.starts_with(u8"..") ? range
                                     : dot;
  case u8'/':
    return str.starts_with(u8"/=") ? div_assignment : div;
  case u8':':
    return str.starts_with(u8"::") ? coloncolon : colon;
  case u8';':
    return str.starts_with(u8";;") ? double_semicolon : semicolon;
  case u8'<':
    return str.starts_with(u8"<=") ? le : langle;
  case u8'=':
    return str.starts_with(u8"===")  ? eqeqeq
           ...
           : str.starts_with(u8">=") ? double_arrow
                                     : assignment;
  case u8'>':
    return str.starts_with(u8">=") ? ge : rangle;
  case u8'?':
    return str.starts_with(u8"?.")   ? safe_call
           : str.starts_with(u8"?:") ? elvis
                                     : quest;
  case u8'@':
    return at;
  case u8'[':
  ...
  // Other keywords are not handled here because we need to parse whole
  // identifiers, then check if they're keywords.
  case u8'a':
    return str.starts_with(u8"as?") ? as_safe : none;
  case u8'b':
    return str.starts_with(u8"break@") ? break_at : none;
  case u8'c':
    return str.starts_with(u8"continue@") ? continue_at : none;
  case u8'r':
    return str.starts_with(u8"return@") ? return_at : none;
  case u8's':
    return str.starts_with(u8"super@") ? super_at : none;
  case u8't':
    return str.starts_with(u8"this@") ? this_at : none;

  case u8'{':
    return lcurl;
  case u8'|':
    return str.starts_with(u8"||") ? disj : none;
  case u8'}':
    return rcurl;
  default:
    return {};
...
Escape_Result match_escape_sequence(const std::u8string_view str) {
  ...
  switch (str[1]) {
  // UniCharacterLiteral.
  // Kotlin strings are superficially UTF-16, so this is the only form of
  // Unicode escape.
  case u8'u':
...
private:
  ...
  void consume_whitespace() {
    const std::size_t space = ascii::length_if(
        remainder, [](char8_t c) { return is_python_whitespace(c); });
    advance(space);
  }
  ...
  bool expect_identifier() {
    if (const std::size_t length = match_identifier(remainder)) {
      ...
      emit_and_advance(length, type ? token_type_highlight(*type)
                                    : Highlight_Type::name);
      return true;
    }
    return false;
    ...
    while (length < remainder.length()) {
      ...
      if (remainder.substr(length).starts_with(terminator)) {
        if (is_multiline) {
          ...
          if (quotes > 3) {
            emit_and_advance(quotes - 3, Highlight_Type::string,
                             Coalescing::forced);
          }
      ...
      if (!is_multiline &&
          (remainder[length] == u8'\r' || remainder[length] == u8'\n')) {
        flush();
        return true;
      }
      ...
      // 4. Any other character is simply part of the string or character
      // literal.
...
bool highlight_kotlin(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                      std::pmr::memory_resource *memory,
                      const Highlight_Options &options) {
