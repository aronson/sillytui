...
std::size_t match_identifier(std::u8string_view str) {
  ...
  return ascii::length_if(
      str, [](char8_t c) { return is_llvm_identifier(c); }, 1);
}
...
Highlight_Type classify_keyword(std::u8string_view str) {
  ...
  const bool is_integer = str[0] == u8'i' && str.length() >= 2 &&
                          ascii::find_if_not(
                              str, [](char8_t c) { return is_ascii_digit(c); },
                              1) == std::u8string_view::npos;
  if (is_integer) {
  ...
  // This is relatively safe considering that normal identifiers start with "@"
  // or "%". It also means that we don't have to maintain a long manual list,
  // only a list of keywords that are treated specially.
  return result != std::ranges::end(keywords) && result->string == str
             ? result->highlight
...
private:
  ...
  bool expect_block_comment() {
    if (const Comment_Result comment = match_block_comment(remainder)) {
      highlight_enclosed_comment(comment, block_comment_prefix.length(),
                                 block_comment_suffix.length());
      return true;
    }
    return false;
  }
  ...
  void consume_identifier() {
    ...
    // This is not strictly accurate because these prefixes indicate
    // local/global symbols, which could include other types of symbols, like
    // global variables.
    if (remainder.starts_with(u8'%')) {
      ...
      ULIGHT_ASSERT_UNREACHABLE(u8"Shouldn't have matched identifier that "
                                u8"starts neither with '%' nor with '@'.");
    }
  }
      ...
      case u8'\\': {
        ...
        if (remainder.starts_with(u8"\\\\")) {
          emit_and_advance(2, Highlight_Type::string_escape);
                       ...
                       match_common_escape<Common_Escape::hex_2>(remainder,
                                                                 1)) {
...
bool highlight_llvm(Non_Owning_Buffer<Token> &out, std::u8string_view source,
                    std::pmr::memory_resource *memory,
                    const Highlight_Options &options) {
