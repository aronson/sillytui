...
std::size_t match_directive_name(const std::u8string_view str) {
  constexpr auto head = [](char8_t c) {
    return is_cowel_directive_name_start(c);
  };
...
std::size_t match_member_name(const std::u8string_view str) {
  constexpr auto predicate = [](char32_t c) {
    return is_cowel_argument_name(c);
  };
  return str.empty() || is_ascii_digit(str[0])
             ? 0
             : utf8::length_if(str, predicate);
}

std::size_t match_escape(const std::u8string_view str) {
  constexpr std::size_t sequence_length = 2;
  if (str.length() < sequence_length || str[0] != u8'\\' ||
      !is_cowel_escapeable(str[1])) {
    return 0;
  }
...
std::size_t match_line_comment(const std::u8string_view str) {
  ...
  static constexpr std::u8string_view comment_prefix_string{
      comment_prefix, std::size(comment_prefix)};
  if (!str.starts_with(comment_prefix_string)) {
    return 0;
  }

  constexpr auto is_terminator = [](char8_t c) {
    return c == u8'\r' || c == u8'\n';
  };
...
std::size_t match_unquoted_string(const std::u8string_view str) {
  constexpr auto predicate = [](char8_t c) {
    return is_cowel_unquoted_string(c);
  };
  return ascii::length_if(str, predicate);
}
...
bool starts_with_escape_or_comment_or_directive(const std::u8string_view str) {
  return str.length() >= 2 && str[0] == u8'\\' &&
         is_cowel_allowed_after_backslash(str[1]);
}
...
std::size_t match_blank(const std::u8string_view str) {
  ...
  while (length < str.length()) {
    ...
    if (const Comment_Result comment =
            match_block_comment(str.substr(length))) {
...
struct [[nodiscard]] Highlighter : Highlighter_Base {
  ...
  bool expect_text(const Text_Kind text_kind, int brace_level) {
    ...
    for (; plain_length < remainder.length(); ++plain_length) {
      ...
      if (c == u8'\\') {
        if (starts_with_escape_or_comment_or_directive(
                remainder.substr(plain_length))) {
          goto after_loop;
        }
        continue;
      }
  ...
  bool expect_escape() {
    ...
    // Even though the escape sequence technically includes newlines and
    // carriage returns, we do not want those to be part of the token for the
    // purpose of syntax highlighting. That is because cross-line tokens are
    // ugly. Therefore, we underreport the escape sequence as only consisting of
    // the backslash.
    ...
    while (!eof()) {
      ...
      if (!member_success) {
        ...
        if (invalid_length == std::u8string_view::npos) {
          ...
          emit_and_advance(1, is_brace ? Highlight_Type::symbol_brace
                                       : Highlight_Type::symbol_parens);
        }
        return true;
      }
    }
  ...
  bool expect_group_member() {
    if (const std::size_t name = match_member_name(remainder)) {
      const std::size_t space_before_equals =
          match_blank(remainder.substr(name));
  ...
  bool expect_directive_call() {
    ...
    const std::size_t blank_after_name =
        match_blank(remainder.substr(name_length));
...
bool highlight_cowel(Non_Owning_Buffer<Token> &out,
